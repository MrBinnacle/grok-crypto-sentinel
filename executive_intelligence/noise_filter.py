...
class ExecutiveNoiseFilter:
    def __init__(self):
        self.noise_patterns = [
            ...
            "circle back", "deep dive", "boil the ocean"
        ]
        
        self.signal_indicators = [
            "revenue impact", "cost reduction", "market share", "customer retention",
            "competitive advantage", "regulatory change", "technology shift"
        ]
    ...
    def filter_executive_content(self, content: str, executive_role: str) -> Dict:
        ...
        noise_score = sum(1 for pattern in self.noise_patterns if pattern in content.lower())
        signal_score = sum(1 for indicator in self.signal_indicators if indicator in content.lower())
    ...
    def prioritize_executive_inputs(self, inputs: List[Dict], executive_role: str) -> List[Dict]:
        ...
        for input_item in inputs:
            ...
            input_item["executive_relevance"] = self._assess_relevance(input_item, executive_role)
        
        # Sort by priority score
        return sorted(inputs, key=lambda x: x["priority_score"], reverse=True)
    
    def generate_executive_summary(self, filtered_inputs: List[Dict], max_items: int = 3) -> Dict:
        ...
        summary = {
            ...
            "high_priority_items": len([i for i in filtered_inputs if i["priority_score"] > 10]),
        ...
        for item in top_items:
            ...
            if item.get("category") == "opportunity":
                summary["opportunities"].append(item.get("title", "Unknown opportunity"))
    ...
    def _extract_insights(self, content: str, executive_role: str) -> List[str]:
        ...
        if executive_role == "ceo" and any(word in content.lower() for word in ["market", "competition", "strategy"]):
            insights.append("Strategic market intelligence detected")
        
        if executive_role == "cfo" and any(word in content.lower() for word in ["cost", "revenue", "margin", "cash"]):
            insights.append("Financial impact information identified")
        
        if executive_role == "coo" and any(word in content.lower() for word in ["process", "efficiency", "operations"]):
