...
def route_signal(
    signal: Union[Dict[str, Any], EnrichedBreakoutSignal], persona: str
) -> None:
    """
    ...
    if os.getenv("NOTION_TOKEN") and os.getenv("NOTION_DB_ID"):
        try:
            ...
            raise ImportError(
                "notion_webhook module or its 'send' method is missing."
            ) from e
        except AttributeError as e:
            raise AttributeError(
                "notion_webhook module must implement a 'send(signal, persona)' method."
            ) from e
