...
class BreakoutDetector:
    ...
    def __init__(self, config: Dict):
        ...
        try:
            self.support_floor = float(
                config.get("codex", {}).get("breakout_watch", {}).get("support_floor")
            )
            self.breakout_zone = float(
                config.get("codex", {}).get("breakout_watch", {}).get("breakout_zone")
            )
    ...
    def detect_breakout(self, symbol: str) -> Optional[BreakoutSignal]:
        ...
        try:
            ...
            # Process volume data
            if not isinstance(volume_data, dict) or not isinstance(
                volume_data.get("volumes"), list
            ):
                return None

            recent_volumes = [
                float(v[1])
                for v in volume_data["volumes"][-HOURS_IN_DAY:]
                if len(v) > 1
            ]
            ...
            if (
                current_price > self.breakout_zone
                and volume_spike_pct > self.volume_threshold
            ):
